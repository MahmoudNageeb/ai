...
registerTool({
  ...
  execute: async (input: { text: string; mode: string; max_words?: number }) => {
    ...
    const resp = await llm.chat(
      ...
      'You are a precise text analysis sub-module of JARVIS. Reply with the analysis only — no preamble.',
    );
