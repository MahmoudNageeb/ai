...
registerTool({
  ...
  execute: (input: { format?: string; timezone?: string } = {}) => {
    const now = new Date();
    if (input.format === 'iso') return { time: now.toISOString() };
    if (input.format === 'unix') return { time: Math.floor(now.getTime() / 1000) };
    try {
      return {
