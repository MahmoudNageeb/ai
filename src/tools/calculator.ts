...
/**
 * Safe math evaluator. NO eval(), NO Function() — strict tokenizer + shunting-yard.
 * Supports: + - * / ^ %, parentheses, unary minus, decimals, and basic functions.
 */
...
type Token =
  ...
  | { type: 'rparen' }
  | { type: 'comma' }
  | { type: 'fn'; value: string; arity?: number };
...
function tokenize(expr: string): Token[] {
  ...
  while (i < s.length) {
    ...
    if ((c >= '0' && c <= '9') || c === '.') {
      ...
      if (Number.isNaN(num)) throw new Error(`Invalid number near "${s.slice(i, j)}"`);
      ...
      else if (FUNCTIONS[name]) tokens.push({ type: 'fn', value: name });
    ...
    else throw new Error(`Illegal character "${c}"`);
  }
  return tokens;
}
  ...
  for (const tok of tokens) {
    if (tok.type === 'num') {
      ...
      if (wasFn && stack.length && stack[stack.length - 1].type === 'fn') {
        const fnTok = stack.pop()! as Extract<Token, { type: 'fn' }>;
...
function evalRpn(rpn: Token[]): number {
  ...
  for (const tok of rpn) {
    if (tok.type === 'num') stack.push(tok.value);
      ...
      const arity = (tok as any).arity ?? 1;
      if (stack.length < arity) throw new Error(`Function ${tok.value} missing args`);
      const args: number[] = [];
