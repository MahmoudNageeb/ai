...
class DecisionEngine {
    ...
    async decide(userInput, availableTools, shortTermContext) {
  ...
  "refined_objective": "clear restatement of the user's true goal"
        ...
        try {
            ...
            return {
                ...
                suggested_tool: typeof result?.suggested_tool === 'string' &&
                    result.suggested_tool !== 'null' &&
                    result.suggested_tool.length > 0
                    ? result.suggested_tool
                    : undefined,
                refined_objective: typeof result?.refined_objective === 'string' && result.refined_objective.length > 0
                    ? result.refined_objective
                    : userInput,
            };
        }
        catch (e) {
            logger_1.logger.warn(`[DecisionEngine] Falling back to heuristic. Reason: ${e.message}`);
            return this.heuristicFallback(userInput, availableTools);
        }
    }
    normalizeMode(raw) {
        const v = String(raw || '').toUpperCase().trim();
        if (v === 'SIMPLE' || v === 'TOOL' || v === 'AGENT')
            return v;
        return 'SIMPLE';
    }
    /** Last-resort classifier when the LLM JSON parse fails. */
    heuristicFallback(userInput, availableTools) {
